...
import com.yjf.common.util.*;
import com.yjf.common.util.conversion.TypeConverterUtils;
...
public class ArrayUtils {
	 ...
	 * 
	 * @param <T> 数组的元素类型。
	 * @param toTypeClass 数组类型(或数组元素类型)的 Class 对象，如果为数组类型 Class，则使用
	 * {@link Class#getComponentType()} 获得元素类型。
	 * @param object 要被复制的对象，如果为数组或者 {@link Collection} 或者 {@link Stack}
	 * ，则将每个放入新数组，如果为其他对象，则直接放入新数组。
	 * @return 对应的数组。
	 ...
	 * {@link Class#getComponentType()} 获得元素类型。
	 * @param object 要被复制的对象，如果为数组或者 {@link Collection} 或者 {@link Stack}
	 * ，则转换每个元素为 toTypeClass 指定的类型放入新数组，如果为其他对象，则直接转换为toTypeClass 指定的类型放入新数组。
	 * @return 对应的数组。
			...
			if (isCast) {
				if (object instanceof Object[]) {
					...
					for (int i = 0; i < t.length; i++) {
						t[i] = (T) TypeConverterUtils.convertValue(Array.get(object, i), toTypeClass);
					}
				}
			} else {
	 ...
	 * @param toArray 目标数组。
	 * @param object 要被复制的对象，如果为数组或者 {@link Collection} 或者 {@link Stack}
	 * ，则将每个元素放入 toArray ，如果为其他对象，则直接放入toArray。
	 ...
	 * @param toArray 目标数组。
	 * @param object 要被复制的对象，如果为数组或者 {@link Collection} 或者 {@link Stack}
	 * ，则转换每个元素为 toArray 的元素类型 的类型放入 toArray ，如果为其他对象，则直接转换为 toArray 的元素类型
	 * 的类型放入toArray。
	 * @throws IndexOutOfBoundsException 如果 toArray 的长度小于 object 对应的长度时。
	 * @throws ArrayStoreException 如果类型转换失败，将不正确的类型放进 toArray 指定的数组元素的数组时。
	 * @throws NullPointerException toArray 为 null 时。
			...
			if (objectClass.isArray()) {
				...
				if (isCast) {
					if (Object[].class.isAssignableFrom(objectClass)) {
						...
						for (int i = 0; i < t.length; i++) {
							toArray[i] = (T) TypeConverterUtils.convertValue(t[i], toArray.getClass()
								.getComponentType());
						}
					} else {
						int length = Array.getLength(object);
						for (int i = 0; i < length; i++) {
							toArray[i] = (T) TypeConverterUtils.convertValue(Array.get(object, i), toArray.getClass()
								.getComponentType());
						}
					}
				} else {
				...
				if ((isCollection = (object instanceof Collection<?>)) || object instanceof Stack<?>) {
					// 为 Collection 或者 Stack
					Iterable<?> iterable = (Iterable<?>) object;
					if ((isCollection ? ((Collection<?>) iterable).size() : ((Stack<?>) iterable).height()) > toArray.length) {
						throw new IndexOutOfBoundsException(String.valueOf(toArray.length));
					}
					int i = 0;
					if (isCast) {
						for (Object obj : iterable) {
							toArray[i] = (T) TypeConverterUtils
								.convertValue(obj, toArray.getClass().getComponentType());
							i++;
						}
					} else {
						for (Object obj : iterable) {
					...
					if (isCast) {
						toArray[0] = (T) TypeConverterUtils.convertValue(object, toArray.getClass().getComponentType());
					} else {
						toArray[0] = (T) object;
					}
				}
	 ...
	 * 
	 * @param array 要判断的数组。
	 * @return 如果数组为 null 或者长度为0，则返回 true。
	 */
	 ...
	 * 
	 * @param array 要判断的数组。
	 * @return 如果数组不为 null 且长度大于0，则返回 true。
	 */
	 ...
	 * <p>
	 * 此方法同 {@link Collection#toArray()}与 {@link Arrays#asList(Object...)}
	 * 一起，充当了基于数组的 API 与基于 collection 的 API 之间的桥梁。返回的集是可序列化的，并且实现了 RandomAccess。
	 * <p>
	 * 该方法返回的 Set 与 {@link Arrays#asList(Object...)} 返回的 List 除了 Set 与 List
	 * 的性质区别外，最主要的区别还有该 Set 在创建后就完全不依赖创建该 Set 的 数组，任何对创建该 Set 的 数组的操作均不会影响到该
	 * Set。
	 * <p>
	 * 由于底层为数组实现，在创建该 Set 和进行 contains(Object) 等方法时，会导致线性时间的操作。在数据量特别大时，请考虑使用别的
	 * Set 实现替代。
	 * 
	 ...
	 * @param <E> 持有元素
	 */
	private static class UnmodifiableArraySet<E> extends AbstractSet<E> implements RandomAccess, Serializable {
	...
	/**
	 * 对 array 进行过滤，新建一个 数组 对结果进行存放并返回，过滤保留的元素为 decider 判定为 true 的元素，如果 decider
	 * 为 null ，则返回所有元素组成的一个新的 数组 。
	 * 
	...
	/*------------------------------------------------------------------------------------
	 *                         以下是 ArrayUtil 的代码复制合并，新增方法请在此注释之上                                             -
	 * ----------------------------------------------------------------------------------*/
	...
	@Deprecated
	public static final Class[] EMPTY_CLASS_ARRAY = ClassUtils.EMPTY_CLASSES;
	
	/** 空的<code>String</code>数组。 */
	@Deprecated
	public static final String[] EMPTY_STRING_ARRAY = StringUtils.EMPTY_STRINGS;
	
	/** 空的<code>long</code>数组。 */
	@Deprecated
	public static final long[] EMPTY_LONG_ARRAY = PrimitiveUtils.EMPTY_LONGS;
	
	/** 空的<code>Long</code>数组。 */
	@Deprecated
	public static final Long[] EMPTY_LONG_OBJECT_ARRAY = PrimitiveUtils.EMPTY_LONG_WRAPPERS;
	
	/** 空的<code>int</code>数组。 */
	@Deprecated
	public static final int[] EMPTY_INT_ARRAY = PrimitiveUtils.EMPTY_INTS;
	
	/** 空的<code>Integer</code>数组。 */
	@Deprecated
	public static final Integer[] EMPTY_INTEGER_OBJECT_ARRAY = PrimitiveUtils.EMPTY_INT_WRAPPERS;
	
	/** 空的<code>short</code>数组。 */
	@Deprecated
	public static final short[] EMPTY_SHORT_ARRAY = PrimitiveUtils.EMPTY_SHORTS;
	
	/** 空的<code>Short</code>数组。 */
	@Deprecated
	public static final Short[] EMPTY_SHORT_OBJECT_ARRAY = PrimitiveUtils.EMPTY_SHORT_WRAPPERS;
	
	/** 空的<code>byte</code>数组。 */
	@Deprecated
	public static final byte[] EMPTY_BYTE_ARRAY = PrimitiveUtils.EMPTY_BYTES;
	
	/** 空的<code>Byte</code>数组。 */
	@Deprecated
	public static final Byte[] EMPTY_BYTE_OBJECT_ARRAY = PrimitiveUtils.EMPTY_BYTE_WRAPPERS;
	
	/** 空的<code>double</code>数组。 */
	@Deprecated
	public static final double[] EMPTY_DOUBLE_ARRAY = PrimitiveUtils.EMPTY_DOUBLES;
	
	/** 空的<code>Double</code>数组。 */
	@Deprecated
	public static final Double[] EMPTY_DOUBLE_OBJECT_ARRAY = PrimitiveUtils.EMPTY_DOUBLE_WRAPPERS;
	
	/** 空的<code>float</code>数组。 */
	@Deprecated
	public static final float[] EMPTY_FLOAT_ARRAY = PrimitiveUtils.EMPTY_FLOATS;
	
	/** 空的<code>Float</code>数组。 */
	@Deprecated
	public static final Float[] EMPTY_FLOAT_OBJECT_ARRAY = PrimitiveUtils.EMPTY_FLOAT_WRAPPERS;
	
	/** 空的<code>boolean</code>数组。 */
	@Deprecated
	public static final boolean[] EMPTY_BOOLEAN_ARRAY = PrimitiveUtils.EMPTY_BOOLEANS;
	
	/** 空的<code>Boolean</code>数组。 */
	@Deprecated
	public static final Boolean[] EMPTY_BOOLEAN_OBJECT_ARRAY = PrimitiveUtils.EMPTY_BOOLEAN_WRAPPERS;
	
	/** 空的<code>char</code>数组。 */
	@Deprecated
	public static final char[] EMPTY_CHAR_ARRAY = PrimitiveUtils.EMPTY_CHARS;
	
	/** 空的<code>Character</code>数组。 */
	@Deprecated
	public static final Character[] EMPTY_CHARACTER_OBJECT_ARRAY = PrimitiveUtils.EMPTY_CHAR_WRAPPERS;
	...
	public static Object[] defaultIfEmpty(Object[] array, Class<?> defaultComponentType) {
		return ((array == null) || (array.length == 0)) ? (Object[]) Array.newInstance(
			ClassUtils.getNonPrimitiveType(defaultComponentType), 0) : array;
	}
