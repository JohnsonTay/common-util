package com.cly.common.util.conversion;

import com.yjf.common.collection.ConcurrentReferenceMap;
import com.yjf.common.collection.SetFromMap;
 ...
 * 
 * <p>
 * 该实现分别支持源类型父类查找和目标类型的子类查找。
 * 
 * <p>
 * <strong>注意：</strong>虽然对注册的转换器保证了强一致的并发性，但是由于在对源类型父类查找和目标类型的子类查找的支持时，
 * 对源类型父类和目标类型子类查找到的转换器该管理器采用了缓存的机制并且没有串行化，
 * 所以该<strong>缓存在极端并发条件下可能出现不一致的情况</strong>。不过从使用场景上看，
 * {@link #register(Class, Class, TypeConverter)} 、 {@link #unregister(Class)} 和
 * {@link #unregister(Class, Class)}
 * 这些会影响缓存的方法并不容易出现极端的并发调用，所以为了性能考虑，该实现允许这种缓存不一致的情况。
 * 
...
public class SimpleTypeConverterManager implements TypeConverterManager {
	...
	/**
	 * 注册 {@link com.yjf.common.util.conversion} 包下的 {@link TypeConverter} 到
	 * converterManager 中。
	 * <p>
	 * 注册清单为：
	 * 
	...
	public <S, T> void register(Class<? extends S> sourceType, Class<T> targetType,
	...
	public <T> Collection<TypeConverter<T>> getTypeConverter(Class<T> targetType) {
		...
		return (Collection<TypeConverter<T>>) (m == null ? Collections.emptyList() : Collections
			.unmodifiableCollection(m.values()));
	}
	...
	static class SourceTypeConverterMap extends ConcurrentReferenceMap<Class<?>, TypeConverter<?>> {
		...
		private final ConcurrentMap<Class<?>, TypeConverter<?>> sourceTypeConverterCache = new ConcurrentReferenceMap<Class<?>, TypeConverter<?>>(
			ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32);
		
		private final Set<Class<?>> notSourceTypeConverters = new SetFromMap<Class<?>, Object>(
			new ConcurrentReferenceMap<Class<?>, Object>(ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32));
		...
		public TypeConverter<?> remove(Object key) {
			...
			if (removedTypeConverter != null) {
				...
				for (Iterator<Class<?>> iterator = this.sourceTypeConverterCache.keySet().iterator(); iterator
					.hasNext();) {
	...
	static class TargetTypeConverterMap extends ConcurrentReferenceMap<Class<?>, SourceTypeConverterMap> {
		...
		private final ConcurrentMap<Class<?>, SourceTypeConverterMap> targetTypeConverterCacheMap = new ConcurrentReferenceMap<Class<?>, SourceTypeConverterMap>(
			ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32);
		
		private final Set<Class<?>> notTargetTypeConverters = new SetFromMap<Class<?>, Object>(
			new ConcurrentReferenceMap<Class<?>, Object>(ReferenceKeyType.WEAK, ReferenceValueType.STRONG, 32));
		...
		public SourceTypeConverterMap remove(Object key) {
			...
			if (removed != null) {
				...
				for (Iterator<Class<?>> iterator = this.targetTypeConverterCacheMap.keySet().iterator(); iterator
					.hasNext();) {
