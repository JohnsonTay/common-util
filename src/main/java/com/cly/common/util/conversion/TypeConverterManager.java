 ...
 * 类型转换器管理器。
 * <p>
 * 注意：由于数组是协变的，所以一维数组可以使用 Object[].class 表示。
 * 
...
public interface TypeConverterManager {
	...
	/**
	 * 通过需要转换到的类型的 {@link Class} 得到对应的类型转换器。如果不存在则返回 则返回
	 ...
	 * @throws IllegalArgumentException 如果 sourceType 或者 targetType 或者
	 * typeConverter 为 null。
	 */
	 ...
	 * @param typeConverter 需要注册的类型转换器实例。
	 * @throws IllegalArgumentException 如果 typeConverter 为 null
	 * 或者提供的源类型或者转换到的目标类型为 null 。
	 */
	 ...
	 * 
	 * @param targetType 需要解除注册的转换到的类型的 {@link Class}。
	 * @return 解除了绑定的类型转换器实例列表，如果解除的是一个未注册的类型转换器，则返回
	 * {@link Collections#emptyList()} 。
	 */
