 ...
 * {@code
 *
 *  @Inject private  DubboRemoteProxyFacotry dubboRemoteProxyFacotry;
 *  GroupService groupService1 = dubboRemoteProxyFacotry.getProxy(GroupService.class, "group1", "1.0");
 * }
 * </pre>
 * 
...
public class DubboRemoteProxyFacotry implements ApplicationContextAware {
	...
	public <T> T getProxy(Class<T> clazz, String group, String version, int timeout) {
		if (Strings.isNullOrEmpty(version) || clazz == null) {
			throw Exceptions.newRuntimeExceptionWithoutStackTrace("获取dubbo服务代理时版本号和类不能为空");
		}
			...
			synchronized (DubboRemoteProxyFacotry.class) {
				...
				if (cachedReferenceConfig == null) {
					...
					Map<String, RegistryConfig> registryConfigMap = applicationContext
						.getBeansOfType(RegistryConfig.class);
					...
					try {
						...
						try {
							...
							throw Exceptions.newRuntimeExceptionWithoutStackTrace("获取dubbo服务失败,销毁Invoker失败:"
																					+ e.getMessage() + ",cause:"
																					+ e.getMessage());
						}
						throw Exceptions.newRuntimeExceptionWithoutStackTrace("获取dubbo服务失败:" + e.getMessage());
					}
